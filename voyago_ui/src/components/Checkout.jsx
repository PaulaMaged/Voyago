...
const Checkout = () => {
  ...
  const fetchCartItems = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8000/api/cart/${roleId}`);
  ...
  const fetchSavedAddresses = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8000/api/orders/tourist-addresses/${roleId}`);
      ...
      setError(err.response?.data?.message || 'Failed to fetch saved addresses');
      setLoading(false);
    }
  };
