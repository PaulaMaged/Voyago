...
const FlightSearch = () => {
  ...
  const handleBookClick = (flightId) => {
    // Toggle the booking form for the clicked flight
    setOpenBookingFlightId((prevId) => (prevId === flightId ? null : flightId));
  };
        ...
        {flights.length > 0 ? (
          flights.map((flight, index) => (
            <div key={index} style={styles.flightCard}>
              ...
              >
                {openBookingFlightId === index ? 'Close Booking' : 'Book'}
              </button>
              {/* Conditionally render the FlightBooking component */}
              {openBookingFlightId === index && (
