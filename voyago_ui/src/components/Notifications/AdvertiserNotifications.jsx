...
const AdvertiserNotifications = () => {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/advertiser/notifications/${advertiserId}`);
        ...
        {notifications.length === 0 && (
          <div className="text-center py-8 text-[var(--textSecondary)]">
            ...
            <p className="text-sm mt-2">
              You'll see notifications here when there are updates to your activities
            </p>
          </div>
        )}
      </div>
