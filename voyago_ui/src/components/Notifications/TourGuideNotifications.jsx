...
const TourGuideNotifications = () => {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/tour-guide/notifications/${tourGuideId}`);
        ...
        {notifications.length === 0 && (
          <div className="text-center py-8 text-[var(--textSecondary)]">
            ...
            <p className="text-sm mt-2">
              You'll see notifications here when there are updates to your itineraries
            </p>
          </div>
        )}
      </div>
